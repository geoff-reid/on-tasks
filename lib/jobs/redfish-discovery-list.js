...
function RedfishDiscoveryListJobFactory(
    ...
    RedfishDiscoveryListJob.prototype.discoverList = function(endpoint) {
        ...
        .spread(function (root, chassis, systems, cooling) {
            var power;
            ...
            return [ root, chassis, systems, cooling, power];
        })
                        ...
                        return Promise.map(data.body.Members, function (elem) {
                            ...
                            .map(function (data) {
                                var node = {
                                    type: 'cooling',
                                    name: data.body.Name,
                                    identifiers: [data.body.ID, self.settings.uri, cooling.Id, coolingTypes[idx]]
                                };
                            ...
                            return Promise.map(data.body.Members, function (elem) {
                                ...
                                .map(function (data) {
                                    var node = {
                                        type: 'power',
                                        name: data.body.Name,
                                        identifiers: [data.body.ID, self.settings.uri, power.Id, powerTypes[idx]]
                                    }
    ...
    RedfishDiscoveryListJob.prototype.createChassis = function (root) {
        ...
        .map(function(data) {
            ...
            var nodeRoot = self.settings.protocol + "://" + self.settings.host + ":" + self.settings.port + data.chassis['@odata.id'];
        ...
        .map(function(data) {
            ...
            .then(function() {
               ...
               // identifiers.push(self.settings.uri)
                var nodeRoot = self.settings.protocol + "://" + self.settings.host + ":" + self.settings.port + data.system['@odata.id'];
        ...
        return self.redfish.clientRequest(root.NetworkDevices['@odata.id'])
            ...
            .map(function(data) {
                ...
                //    .then(function() {
                        ...
                        // todo find a better unique identifier
                        // identifiers.push(self.settings.uri)
                var nodeRoot = self.settings.protocol + "://" + self.settings.host + ":" + self.settings.port + data.network['@odata.id'];
